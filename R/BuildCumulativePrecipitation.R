...
cum.precip.graph <- daily.summary.stats |>
  ...
  scale_x_continuous(expand = expansion(c(0, 0.04)),
                     ...
                     name = NULL) +
  labs(title = "Cumulative annual precipitation at Charlottesville's McCormick Observatory",
  ...
  annotate("label", x = 10, y = 60, label = str_wrap(median.annotation, 40),
...
ggsave("graphs/Precip_2026.png", plot = cum.precip.graph,
       width = 8, height = 4)
